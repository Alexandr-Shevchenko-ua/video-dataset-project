...
def _resolve_indices(
    total: int, selection: DatasetSelectionConfig, *, seed: int
) -> List[int]:
...
def download_selected_zips(config: PipelineConfig) -> List[ZipItem]:
    ...
    This uses the standard datasets caching directory under raw_cache_dir.
    """
...
def stream_zips(config: PipelineConfig) -> Iterator[ZipItem]:
    ...
    Useful when disk budget is tight (<5 GB).
    """
