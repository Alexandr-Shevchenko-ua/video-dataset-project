...
class MetadataWriter:
    ...
    def _serialize_record(self, record: FrameRecord) -> Dict[str, object]:
        data = asdict(record)
        resolution = data.pop("resolution", (0, 0))
        data["resolution_height"], data["resolution_width"] = (int(resolution[0]), int(resolution[1]))
        data["quality_scores"] = json.dumps(record.quality_scores, sort_keys=True)
        data["augmentations"] = json.dumps(record.augmentations, default=str)
        return data

    def _flush_buffer(self) -> None:
        if not self._buffer:
            return
        rows = [self._serialize_record(record) for record in self._buffer]
