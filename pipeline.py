...
from dataset_loader import ZipItem, cleanup_zip_item, download_selected_zips, stream_zips
...
def _ensure_path_separation(config: PipelineConfig) -> None:
    ...
    if raw == out or raw == temp or out == temp:
        raise ValueError("raw cache, temp extract, and output paths must differ")
...
def run_pipeline(config: PipelineConfig, progress_cb: Optional[Callable[[ProgressUpdate], None]] = None) -> Path:
    ...
    def _writer() -> MetadataWriter:
        nonlocal metadata_writer
